...
const defaultTestimonial: Testimonial = {
    ...
    content: "",
    position: "", // Added missing required field
    rating: 5,
    isFeatured: false,
    isApproved: false,
    showOnTestimonials: false, // Added missing required field
    showOnHome: false, // Added missing required field
    createdAt: new Date().toISOString(),
    mediaId: undefined,
    media: undefined, // Default image
    user: undefined,
    eventId: undefined,
    // event: undefined,
};
...
const EditTestimonialPage = () => {
    ...
    const [userImage, setUserImage] = useState<File | null>(null); // State to store image file
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const testimonialPage = await api.get(`/testimonials/${testimonialId}`);
                const formattedData = {
                    ...testimonialPage
                };
                setFormData(formattedData);

                // Set image preview URL if media exists
                if (testimonialPage.media && testimonialPage.media.url) {
                    setImagePreviewUrl(testimonialPage.media.url);
                    setIsVideo(isVideoFile(testimonialPage.media.url));
                }
            } catch (err: any) {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            reader.onloadend = () => {
                ...
                // Remove mediaId when new image is selected
    ...
    const edit = async ()=> {
        try {
            ...
            newData.file = userImage;

            const data = await api.patch(`/testimonials/${testimonialId}`, newData); // Call the get API endpoint
    ...
    const save = async () => {
        try {
            ...
            const data = await api.post(`/testimonials`, newData); // Call the get API endpoint
            setFormData(data); // Update the users state with the fetched data
            // router.push('/users');
        } catch (err: any) {
            console.error("Error fetching users:", err.message);
                            ...
                            <div className="mb-4.5">
                                ...
                                <div className="relative z-20 bg-transparent dark:bg-form-input">
                                    <select
                                        ...
                                        onChange={(e) => setFormData({ ...formData, testimonialType: e.target.value as TestimonialType })}
                                ...
                                <>
                                    ...
                                    <div className="mb-4.5">
                                        ...
                                        >
                                            ...
                                            <option value={TestimonialCategory.COACH}>Trener</option>
                                        </select>
                                    </div>
