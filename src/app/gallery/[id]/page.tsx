...
export default function EditGalleryPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const selectedMediaIds = galleryResponse.media.map(
                    (item: { mediaId: number }) => item.mediaId
                );

                // Set form data from gallery response
                setFormData({
                    ...galleryResponse,
                    selectedImages: selectedMediaIds
                });
            } catch (err: any) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await apiJson.patch(`/galleries/${id}`, galleryData);
            toast.success("Gallery updated successfully");
            router.push("/gallery");
        } catch (error: any) {
            console.error("Error updating gallery:", error);
            toast.error("Failed to update gallery: " + (error.message || "Unknown error"));
        } finally {
    ...
    return (
        <DefaultLayout>
            <div className="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
                <Breadcrumb pageName="Edit Gallery" />
                <ToastContainer />
