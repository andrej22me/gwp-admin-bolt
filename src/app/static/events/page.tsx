...
const EditPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const contactPage = await api.get("/pages/events");
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await api.patch("/pages/events", contactPageData); // Adjust the endpoint as per your backend
            toast.success("Saved!", {})
        } catch (err: any) {
    ...
    const handleComponentFileChange = (sectionIndex: number, componentIndex: number, e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            reader.onloadend = () => {
                ...
                if (newSections[sectionIndex]?.components[componentIndex]) {
                    newSections[sectionIndex].components[componentIndex].description = reader.result as string;
                }
                setFormData(prev => ({
                    ...prev,
    ...
    return (
        <DefaultLayout>
            ...
            <div className="mx-auto max-w-270">
                <h1 className="text-3xl font-bold mb-4 dark:text-white">Events</h1>
